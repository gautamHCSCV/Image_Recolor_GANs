...
def visualize(model, data, save=True,epoch=1):
    model.net_G.eval()
    with torch.no_grad():
    ...
    plt.show()
    if save:
        fig.savefig(f"colorization_{epoch+1}.png")
...
def train_model(model, train_dl, epochs, display_every=300):
    data = next(iter(val_dl)) # getting a batch for visualizing the model output after fixed intrvals
    save_at = [0,9,19,29,39]
    for e in range(epochs):
        ...
        for data in tqdm(train_dl):
            ...
            if i % display_every == 0:
                ...
                if e in save_at:
                    save_at.remove(e)
                    visualize(model, data, save=True, epoch=e) # function displaying the model's outputs
